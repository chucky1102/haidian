...
| K5 原点社区概念轴测 | `assets/media/k5-origin-community-axonometric.webp` | OpenAI 内置图像生成;以账号所有者 2026-08-24 四张自有踏勘照作为在地材质与铁路遗产线索,不使用同行方案图片;非现场照片、非测绘底图、非审批效果图 |
...
三处授权与证据边界须专门说明:声景**未使用任何真实馆藏钟声采样**——大钟寺古钟的采样须先取得博物馆授权,授权到手之前,合成音色是唯一诚实的做法;短片**刻意不使用系统 TTS 人声**,因为合成语音的再分发授权在各平台口径不一,宁可无人声;K5 轴测中的人物、建筑与空间关系均为生成式概念表达,四张踏勘照只提供场地气质与可见遗产线索,不能据图反推现场几何。全部资产均为概念示意,不表述为官方渲染、测绘成果或现状记录。
...
![K5北京AI原点社区原创AI轴测概念场景:铁路遗产绿脊、原点广场、可逆AI服务亭与多年龄无障碍公共生活并置;非现场照片、非测绘底图、非审批效果图](assets/media/k5-origin-community-axonometric.webp)

上图把原点社区的空间判断压缩为一个可读场景:铁路遗产不是布景而是公共主脊,AI设施旁置于连续步行净线之外,老人、轮椅使用者、家庭、研究者、骑行者与配送骑手共享同一日常空间。图像由 OpenAI 内置图像生成,以账号所有者四张自有踏勘照作在地视觉线索;未使用同行方案图面,不新增任何几何、指标或现场证据主张 [source:AI-IMAGEGEN-V19-K5]。

...
| 表达完整度 `expression_completeness` 15% | 中英双语全套:正文×2、十图、展示页×2、A3/A0×2、原创Logo、K5概念轴测、展厅封面、交互漫游(Canvas)、原创声景、60秒短片、渲染HTML | `manifest.json`;`visual/index.html#explorer`;`assets/media/` | 全部为已登记生成方法的概念表达,非官方效果图、非测绘、非现状记录 |
...
上图同时是全带总览:图中虚线为临时粗略边界(不得作红线理解),彩色实体要素为本包设计层;灰色建筑肌理来自 2026-08-31 获取的 OpenStreetMap 建筑轮廓,**只作低对比背景定位**,不用于边界、面积、指标、现状建筑盘点或工程判断 [source:SRC-OSM-COPYRIGHT]。证据链的组织方式是:GeoJSON 图层为权威空间数据 [data:geometry/site_boundary.geojson#SITE-001],metrics 为权威指标,三大矩阵为任务、标准与深度的映射表,正文负责把这些机器证据讲成人能读懂的设计判断;图与 PDF 只是解释层。
...
**五件多模态表达资产,逐件登记来源与表达边界。**官方多模态指引要求"为人而设计,而不只为校验器而设计";本方案据此同时保留可复算图件、交互与声画资产,并在 v1.9 新增一张原创空间概念轴测。AI 生成、用户自有照片、开放数据与纯代码资产严格分账,任何一件都不能越级充当测绘、现状或审批证据:

| 资产 | 文件 | 做法与边界 |
| --- | --- | --- |
| 展厅封面 | `assets/media/cover.webp` | 原有人字线代码绘制识别面板与 K5 原创概念轴测组合;概念示意,非官方效果图 |
| K5 原点社区概念轴测 | `assets/media/k5-origin-community-axonometric.webp` | OpenAI 内置图像生成;经 11 份一手资料研究 10 组国际创新街区、铁路遗产更新与混合社区后独立转译,未向生成器提供案例图、踏勘照或同行方案图片;非现场照片、非测绘底图、非审批效果图 |
...
三处授权与证据边界须专门说明:声景**未使用任何真实馆藏钟声采样**——大钟寺古钟的采样须先取得博物馆授权,授权到手之前,合成音色是唯一诚实的做法;短片**刻意不使用系统 TTS 人声**,因为合成语音的再分发授权在各平台口径不一,宁可无人声;K5 轴测中的人物、建筑与空间关系均为生成式概念表达,生成时未输入任何案例图、踏勘照或同行方案图面,不能据图反推现场几何。全部资产均为概念示意,不表述为官方渲染、测绘成果或现状记录。
...
![K5北京AI原点社区原创AI轴测概念场景:校城协作街与铁路记忆绿脊交汇,混合街区、开放首层、共享院落和可逆AI设施共同构成生活学习环;非现场照片、非测绘底图、非审批效果图](assets/media/k5-origin-community-axonometric.webp)

上图不再把原点社区画成一条经过美化的公园绿道,而是一个完整城市片段:南北向铁路记忆绿脊与东西向校城协作街在 K5 公共客厅交汇,既有科研楼更新、混合居住、社区服务、开发者首层、共享院落与轻量协作桥共同形成“生活学习环”;钢轨只以铺地细线与再利用构件留下记忆,AI 服务亭和测试带退到连续无障碍公共路径之外。图像由 OpenAI 内置图像生成,生成时未输入案例图、踏勘照或同行方案图面,不新增任何几何、指标或现场证据主张 [source:AI-IMAGEGEN-V19-K5]。
